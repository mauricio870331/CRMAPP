...
//Insert Estados:
    $SQL_INSERT_E = "INSERT INTO estados_personas (id_estado,id_persona,fecha_registro, update_by) "
            . "VALUES (1,(select id from personas where ss = '" . $_POST['ss'] . "'),NOW(), " . $_SESSION['obj_user'][0]["id"] . ")";
    $rows = $con->exec($SQL_INSERT_E);

    if ($rows > 0) {
        $ruta = "../../Documentos/" . $_POST["ss"];
        if (!file_exists($ruta)) {
